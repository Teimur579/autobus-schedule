...
const getNextDeparture = (firstDepartureTime, frequencyMinutes) => {
    ...
    if (departure > endOfDay) {
        departure = departure.startOf('day').plus({days: 1}).set({hour: 23, minute: 59});
    };
    ...
    while (now > departure) {
        ...
        if (departure > endOfDay) {
            departure = departure.startOf('day').plus({days: 1}).set({hours: 23, minutes: 59});
        };
    };
...
const sortBuses = (buses) => {
    ...
    return [...buses].sort((a, b) => new Date(`${a.nextDeparture.date}T${a.nextDeparture.time}Z`) - new Date(`${b.nextDeparture.date}T${b.nextDeparture.time}Z`));
